...
def addpost(request):
    if request.method == "POST":
        ...
        if postform.is_valid():
            title = platform.cleaned_data['btitle']
            name = platform.cleaned_data['bname']
            gender = request.POST.get('bgender', None)
            email = postform.cleaned_data['bemail']
            content = postform.cleaned_data['bcontent']
            unit = models.BoardUnit.objects.create(bname=name, bgender=gender, btitle=title, bemail=email, bcontent=content, bresponse='')
            uit.save()
            message = '已儲存...'
